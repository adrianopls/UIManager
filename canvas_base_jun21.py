...
"""
class CanvasBaseControllerJun21(UIControllerObject):
    tid = None
...
class CanvasBaseViewJun21(UIViewObject, FigureCanvas):  
    tid = None
    ...
    def __init__(self, controller_uid):
        ...
        self.gridspec = gridspec.GridSpec(ncols=6, nrows=6, figure=self.figure)
        
        print("\n\n")
        print(self.gridspec[0:2, 4:6])
        #print("\n\n")
        
        #self.figure.add_subplot(self.gridspec[0:2, 4:6])
    ...
    def PostInit(self):
        ...
        #
        self.base_axes = self.figure.add_subplot(self.gridspec[:, 0:4])#,
        #
...
#        self.figure.add_axes(self.base_axes)
        ...
        self.ax0 = self.figure.add_subplot(self.gridspec[0:2, 4:6])
        self.ax1 = self.figure.add_subplot(self.gridspec[2:4, 4:6])
        self.ax2 = self.figure.add_subplot(self.gridspec[4:6, 4:6])
    ...
    def on_press(self, event):
        ...
        if event.guiEvent.GetSkipped():
            if event.inaxes == self.plot_axes:
                xdata = event.xdata
                ydata = event.ydata
                #print(int(xdata), int(ydata))
                UIM = UIManager()
                parent_uid = UIM._getparentuid(self._controller_uid)
                parent_controller = UIM.get(parent_uid) 
                parent_controller.view.on_button_press(event)
                #print(parent_uid)
